...
@given('the API is accessible at "{url}"')
...
@given('I have submitted a message with content "{content}" from sender "{sender}"')
...
def step_given_multiple_messages_submitted(context: Any) -> None:
    ...
    for message_data in messages:
        response: requests.Response = requests.post(f"{BASE_URL}/messages", json=message_data)
        assert response.status_code == 201
        context.submitted_messages.append(response.json())


@when('I submit a message with content "{content}" from sender "{sender}"')
...
@when('I request status for message ID "{message_id}"')
...
@when('I request updates for message ID "{message_id}"')
...
@then('the status should be either "{status1}" or "{status2}"')
...
@then('eventually the message should be processed')
...
@then('the response should include total message count')
...
@then('the service should respond with status "{expected_status}"')
...
@then('I should get a {status_code:d} error')
